...
- 투명도
  - Opacity(불투명도)
    ...
    - 0~1 사이의 소수점 값 사용
  - Transparent(투명도)
    - 단일 속성값(투명할거냐, 말거냐 2가지만)
  - Alpha(알파채널)
    - 0~1 사이의 소수점 값 사용
    - rgba() 함수 사용 <!-- rgba는 글자는 그대로, 배경만 투명해짐>
...
- 텍스트 정렬 : text-align
- 왼쪽(left), 가운데(conter), 오른쪽(right), 양쪽 정렬(justify)
...
- 글꼴 종류 : font-family
  - 앞에서부터 차례대로 설치된 폰트를 찾아서 설치되어 있는 폰트를 화면에 랜더링 함
  - Fallback 기능
...
- 브라우저가 폰트를 서버에서 찾도록 하는 기능이 만들어짐 => web font
...
- 로컬 서버에 웹 폰트 파일을 업로드해서 사용하는 방법
  ...
  - 영어 : roboto, lato, open sans, montserrat
  - 고딕체 : sans-serif
  - 명조체 : serif
...
## 네이밍 표기법 : 두개 이상의 단어로 네이밍을 할 때, 단어 사이의 구분
...
- %
  ...
  - height : 자식요소를 기준으로 맞춰질 => % 지정이 적용되지 않음
